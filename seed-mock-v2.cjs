...
// Using Regex to find arrays exactly as they exist now (they could have content or be empty)
content = content.replace(/export const INITIAL_BOOKINGS: Booking\[\] = \[.*?\];/s, 'export const INITIAL_BOOKINGS: Booking[] = ' + bookings + ';');
content = content.replace(/export const INITIAL_PAYMENTS: Pembayaran\[\] = \[.*?\];/s, 'export const INITIAL_PAYMENTS: Pembayaran[] = ' + payments + ';');
content = content.replace(/export const INITIAL_INVOICES: Invoice\[\] = \[.*?\];/s, 'export const INITIAL_INVOICES: Invoice[] = ' + invoices + ';');
content = content.replace(/export const INITIAL_MAINTENANCE: MaintenanceRecord\[\] = \[.*?\];/s, 'export const INITIAL_MAINTENANCE: MaintenanceRecord[] = ' + maintenance + ';');
content = content.replace(/export const INITIAL_REFUNDS: Refund\[\] = \[.*?\];/s, 'export const INITIAL_REFUNDS: Refund[] = ' + refunds + ';');
content = content.replace(/export const INITIAL_NOTIFICATIONS: AppNotification\[\] = \[.*?\];/s, 'export const INITIAL_NOTIFICATIONS: AppNotification[] = ' + notifications + ';');

content = content.replace(/'autorent_initialized', 'v8'/g, "'autorent_initialized', 'v9'");
content = content.replace(/'autorent_initialized' === 'v8'/g, "'autorent_initialized' === 'v9'");
content = content.replace(/'v8'/g, "'v9'"); 

fs.writeFileSync(dataFile, content);

console.log('Mock data injected and version updated to v9!');
