...
export default function DashboardAdmin({
  ...
  const submitApproveRefund = () => {
    ...
    if (targetBooking) {
      ...
      const newInvoice: Invoice = {
        id: 'inv_rfd_' + targetBooking.bookingCode,
        invoiceCode: 'INV-RFD-' + targetBooking.bookingCode,
        bookingId: targetBooking.id,
        bookingCode: targetBooking.bookingCode,
        userId: targetBooking.userId,
        userNama: targetBooking.userNama,
        layanan: 'Refund Pembatalan',
        rincianItem: 'Pengembalian Dana Pembatalan Booking ' + targetBooking.bookingCode,
        subtotal: ref.nominalRefund,
        denda: 0,
        total: ref.nominalRefund,
        terbayar: ref.totalDibayar,
        sisa: 0,
        status: 'refund',
        tanggalDibuat: new Date().toISOString().substring(0, 10),
        metodePembayaran: targetBooking.metodePembayaran || 'gateway',
  ...
  const handleRejectRefund = (ref: Refund) => {
    if (!confirm(`Tolak pengajuan refund ${ref.id}?`)) return;
    ...
    if (targetBooking) {
      const updatedBookings = bookings.map(b => {
        if (b.id === ref.bookingId) {
          ...
          return { 
            ...b, 
            status: isLunas ? 'Lunas' as const : 'DP Dibayar' as const,
    ...
    onAddNotification(
      'Refund Ditolak', 
      `Pengajuan refund ${ref.id} untuk customer ${ref.userNama} telah ditolak. Reservasi tetap aktif.`, 
      'warning',
      ref.userId
    );
  };
  ...
  const handleKonfirmasiPembayaran = (bk: Booking, metode: 'Transfer' | 'QRIS' | 'Cash', nominal: number) => {
    ...
    const newStatusPembayaran = isFullyPaid ? 'Lunas' as const : 'DP Dibayar' as const;
    ...
    const updatedBookings = bookings.map(b => {
      if (b.id === payment.bookingId) {
        return { 
          ...
          statusPembayaran: 'Ditolak' as const
        };
      }
      return b;
    });
  ...
  const handleReturnVehicle = (bookingId: string, retDate: string, retTime: string, notes: string, carReturned: boolean, guaranteeReturned: boolean) => {
    ...
    syncLegacyData(bookingId, bk.jenisJaminan || '', guaranteeReturned ? 'dikembalikan' : 'diverifikasi', 0, {
      ...
      dendaCalculated: penalty
    });

    onAddNotification(
      'Pengembalian Kendaraan Dikonfirmasi',
      `Unit ${bk.mobilNama} telah dikembalikan. Denda: Rp ${penalty.toLocaleString('id-ID')}. ${isFullyPaid ? 'Tagihan lunas.' : 'Menunggu Pelunasan Denda.'}`,
