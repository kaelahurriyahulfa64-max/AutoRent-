...
export default function DashboardCustomer({
  ...
  const [dynamicPaymentUrl, setDynamicPaymentUrl] = useState<string>('');
  const [linkApiError, setLinkApiError] = useState<string>('');
  ...
  // Generate dynamic Sandbox payment link via API proxy using Midtrans Snap API
  React.useEffect(() => {
    if (selectedBookingForPay && paymentMethod === 'Payment Gateway') {
      ...
      const generateLink = async () => {
        const orderId = `${selectedBookingForPay.bookingCode}-${Date.now().toString().slice(-4)}`;
        ...
        try {
          ...
          if (!response.ok) {
            ...
            throw new Error(`Midtrans API Error (HTTP ${response.status}): ${errText}`);
          }

          const data = await response.json();
          if (data && data.redirect_url) {
            setDynamicPaymentUrl(data.redirect_url);
          } else {
            throw new Error('Response URL tidak ditemukan');
          }
        } catch (error: any) {
          console.error('Error generating dynamic Midtrans link:', error);
          setLinkApiError(error.message || 'API Error');
          setDynamicPaymentUrl('');
        } finally {
  ...
  // Client-side Webhook Bypass for Localhost
  React.useEffect(() => {
    ...
    const pollStatus = async () => {
      ...
      try {
        ...
        console.error('Local webhook bypass polling error:', err);
      }
    };
  ...
  }, [activeOrderId, settings.midtransServerKey]);
  ...
  const handlePaymentSubmit = (e: React.FormEvent) => {
    ...
    const newPayment: Pembayaran = {
      ...
      userId: currentUser.id,
      userNama: currentUser.name,
      tipeBayar: paymentAmount >= selectedBookingForPay.totalBayar ? 'lunas_full' : 'dp',
    ...
    const updatedBookings = bookings.map(b => {
      if (b.id === selectedBookingForPay.id) {
        if (isGateway) {
          const isDenda = b.statusPembayaran === 'Menunggu Pelunasan Denda' || b.status === 'Menunggu Pelunasan Denda';
          const totalFinal = b.totalAkhir || b.totalBayar;
          const newJumlahBayar = b.jumlahBayar + paymentAmount;
          const newSisa = Math.max(0, totalFinal - newJumlahBayar);
          const isLunas = newSisa <= 0;
          return { 
            ...b, 
            status: isDenda 
              ? ('Menunggu Pelunasan Denda' as const) 
          ...
          return { 
            ...b, 
            status: 'Menunggu Verifikasi Admin' as const,
    ...
    if (isGateway) {
      ...
      if (!inv) {
        ...
        const alreadyPaid = selectedBookingForPay.jumlahBayar + paymentAmount;
        if (alreadyPaid >= selectedBookingForPay.totalBayar) {
          invoiceStatus = 'lunas';
        }
        ...
        const updatedInvoices = invoices.map(i => {
          if (i.bookingId === selectedBookingForPay.id) {
            return {
              ...
              sisa: Math.max(0, i.total - (i.terbayar + paymentAmount)),
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-white p-3.5 rounded-xl border border-slate-100 flex flex-col justify-between">
                      ...
                      <div className="flex gap-2">
                        <div className="flex-1 py-1.5 px-2.5 rounded-lg border text-center font-bold text-[10px] bg-blue-600 text-white border-blue-600 shadow-xs">
                          Online Gateway
                        </div>
                      </div>
                    </div>
                  ...
                  {paymentMethod === 'Payment Gateway' ? (
                    <div className="bg-white border border-slate-100 rounded-xl p-5 space-y-4 shadow-2xs">
                      ...
                      <div className="flex flex-col gap-2">
                        <a
                          href={dynamicPaymentUrl || '#'}
                          target="_blank"
                          rel="noreferrer"
                          onClick={(e) => {
                            if (isLoadingLink || !dynamicPaymentUrl) {
                              e.preventDefault();
                              alert('Sedang menyiapkan link pembayaran dari Midtrans Sandbox, silakan tunggu...');
                            }
                          }}
                          className={`w-full text-white font-extrabold py-3 rounded-xl text-center transition-all flex items-center justify-center gap-1.5 cursor-pointer text-xs ${
                            isLoadingLink || !dynamicPaymentUrl
                              ? 'bg-slate-400 cursor-not-allowed'
                              : 'bg-blue-600 hover:bg-blue-700 shadow-md shadow-blue-500/10'
                          }`}
                        >
                          {isLoadingLink ? 'Menyiapkan Tautan...' : 'Bayar Sekarang (Buka Link Sandbox)'} <ChevronRight className="w-4 h-4" />
                        </a>
                        ...
                        {!isLoadingLink && linkApiError && (
                          <span className="text-rose-600 text-[10px] text-center block font-semibold">
                            ❌ Gagal menyiapkan pembayaran Sandbox: {linkApiError}
                          </span>
                        )}
                        {!isLoadingLink && dynamicPaymentUrl && !linkApiError && (
                          <div className="flex flex-col gap-2">
                            <span className="text-emerald-600 text-[10px] text-center block font-semibold">
                              ✓ Tautan otomatis sukses! Nominal Rp {paymentAmount.toLocaleString('id-ID')} siap dibayar.
                            </span>
                            <button
                              type="button"
                              onClick={async () => {
                    ...
                    <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-2xl p-5 text-xs space-y-4 text-left">
                      <div className="space-y-4">
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          >
                            {isLoadingLink ? 'Menyiapkan Tautan...' : 'Bayar Sekarang via Midtrans Sandbox'} <ChevronRight className="w-4 h-4" />
                          </a>
                          ...
                          {!isLoadingLink && dynamicPaymentUrl && !linkApiError && (
                            <span className="text-emerald-600 text-[10px] text-center block font-semibold">
                              ✓ Tautan otomatis sukses! Siap dibayar.
                            </span>
                          )}
                        </div>
                      </div>
