...
export default function App() {
  ...
  const generateInvoiceFromBooking = (
    booking: Booking,
    amount: number,
    ...
    const rincianText = booking.layanan === 'rental'
      ? `Sewa Lepas Kunci ${booking.mobilNama} (${booking.durasiHari || 0} hari)`
      : booking.layanan === 'rental_driver'
      ? `Sewa ${booking.mobilNama} (${booking.durasiHari || 0} hari) + Jasa Driver ${booking.driverNama}`
      : `Jasa Driver Profesional ${booking.driverNama}`;

    const invCode = `INV/${new Date().getFullYear()}/${booking.bookingCode}`;
    ...
    return {
      ...
      userNama: booking.userNama,
      layanan: booking.layanan === 'rental' ? 'Rental Mobil' : booking.layanan === 'rental_driver' ? 'Rental + Driver' : 'Jasa Driver',
      rincianItem: rincianText,
      subtotal: booking.totalBayar,
      denda: booking.denda || 0,
      biayaTambahan: booking.biayaTambahan || 0,
      total: booking.totalBayar,
      totalNominal: booking.totalBayar,
      terbayar: amount,
      sisa: Math.max(0, booking.totalBayar - amount),
      status: amount >= booking.totalBayar ? 'lunas' : 'dp_lunas',
      tanggalDibuat: new Date().toISOString().substring(0, 10),
      tanggal: new Date().toISOString().substring(0, 10),
      metodePembayaran: paymentMethod,
      tanggalPembayaran: transactionTime || new Date().toISOString().replace('T', ' ').substring(0, 16),

      // Required Audit fields
      invoiceId: invId,
      invoiceNumber: invCode,
      customerName: booking.userNama,
      customerPhone: booking.userPhone || '',
      vehicleName: booking.mobilNama || '',
      driverName: booking.driverNama || '',
      rentalStartDate: booking.tanggalMulai || '',
      rentalEndDate: booking.tanggalSelesai || '',
      additionalCharges: (booking.denda || 0) + (booking.biayaTambahan || 0),
      totalAmount: booking.totalBayar,
      paymentDate: transactionTime || new Date().toISOString().replace('T', ' ').substring(0, 16),
      invoiceStatus: amount >= booking.totalBayar ? 'lunas' : 'dp_lunas',
  ...
  const handlePaymentSuccess = ({
    ...
    // Retry / Double-Click protection
    if (paymentIdToApprove) {
      const existingPayment = currentPayments.find(p => p.id === paymentIdToApprove);
      if (existingPayment && existingPayment.status === 'disetujui') {
    ...
    if (midtransOrderId) {
      const alreadyProcessed = currentPayments.some(p => p.midtransOrderId === midtransOrderId && p.status === 'disetujui');
      if (alreadyProcessed) return false;
    }

    const finalTotal = bk.totalAkhir || bk.totalBayar;
    const newJumlahBayar = bk.jumlahBayar + amount;
    const isLunas = newJumlahBayar >= finalTotal;

    // 1. Update/Create Payment record
    let updatedPayments = [...currentPayments];
    if (paymentIdToApprove) {
      updatedPayments = updatedPayments.map(p => {
        if (p.id === paymentIdToApprove) {
          return { ...p, status: 'disetujui' as const };
        }
        return p;
      });
    } else {
      const resolvedTipeBayar = tipeBayar || (bk.jumlahBayar === 0
        ? (isLunas ? 'lunas_full' : 'dp')
        : (isLunas ? 'lunas' : 'pelunasan'));

      const newPayment: Pembayaran = {
        id: `p_success_${Date.now()}_${Math.random().toString(36).substring(2, 5)}`,
        bookingId: bk.id,
        ...
        tanggalBayar: transactionTime || new Date().toISOString().replace('T', ' ').substring(0, 16),
        status: 'disetujui',
        midtransOrderId: midtransOrderId || undefined
      };
      updatedPayments = [newPayment, ...updatedPayments];
    }

    // 2. Update Booking Status
    const isDenda = tipeBayar === 'denda' || bk.statusPembayaran === 'Menunggu Pelunasan Denda' || bk.status === 'Menunggu Pelunasan Denda';
    ...
    const newBookingStatus = isDenda 
      ...
      : ('Dikonfirmasi' as const); // Maps to 'Menunggu Pengambilan' in UI
    ...
    const updatedBookings = currentBookings.map(b => {
      if (b.id === bk.id) {
        return {
          ...
          sisaPelunasan: Math.max(0, finalTotal - newJumlahBayar),
          status: newBookingStatus,
          statusPembayaran: newPaymentStatus,
          midtransOrderId: midtransOrderId || b.midtransOrderId,
    ...
    // 3. Create or Update Invoice (Idempotent)
    let updatedInvoices = [...currentInvoices];
    const existingInvIndex = updatedInvoices.findIndex(inv => inv.bookingId === bk.id);
    if (existingInvIndex >= 0) {
      const inv = updatedInvoices[existingInvIndex];
      const newTerbayar = inv.terbayar + amount;
      updatedInvoices[existingInvIndex] = {
        ...inv,
        terbayar: newTerbayar,
        sisa: Math.max(0, inv.total - newTerbayar),
        status: newTerbayar >= inv.total ? 'lunas' : 'dp_lunas',
        invoiceStatus: newTerbayar >= inv.total ? 'lunas' : 'dp_lunas',
        metodePembayaran: paymentMethod,
        tanggalPembayaran: transactionTime || new Date().toISOString().replace('T', ' ').substring(0, 16),
        paymentDate: transactionTime || new Date().toISOString().replace('T', ' ').substring(0, 16)
      };
    } else {
      const newInvoice = generateInvoiceFromBooking(bk, amount, paymentMethod, transactionTime);
      updatedInvoices = [newInvoice, ...updatedInvoices];
    }

    // 4. Update Car and Driver statuses (as per existing business rules)
    ...
    const customerNotif: AppNotification = {
      ...
      message: `Pembayaran booking ${bk.bookingCode} sebesar Rp ${amount.toLocaleString('id-ID')} berhasil. Invoice telah diperbarui.`,
      type: 'success',
      read: false,
    ...
    const adminNotif: AppNotification = {
      ...
      message: `Pembayaran booking ${bk.bookingCode} sebesar Rp ${amount.toLocaleString('id-ID')} berhasil diterima.`,
      type: 'success',
      read: false,
      timestamp: formattedTimestamp
    };
    ...
    const ownerNotif: AppNotification = {
      ...
      message: `Transaksi baru untuk booking ${bk.bookingCode} berhasil tercatat.`,
      type: 'success',
      read: false,
      timestamp: formattedTimestamp
    };
    ...
    // 6. Synchronous Persistence to localStorage
  ...
  const processMidtransWebhook = async (wh: { id?: string; orderId: string; transactionStatus: string; grossAmount: number; paymentType: string; transactionTime: string; }): Promise<boolean> => {
    const currentBookings = getStoredState('bookings', INITIAL_BOOKINGS);
    const bk = currentBookings.find(b => wh.orderId && (
    ...
    if (wh.transactionStatus === 'settlement' || wh.transactionStatus === 'capture') {
      return handlePaymentSuccess({
        bookingId: bk.id,
        amount: wh.grossAmount,
        paymentMethod: wh.paymentType ? wh.paymentType : 'Payment Gateway',
